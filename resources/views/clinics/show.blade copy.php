...
<div class="d-flex" style="    flex-direction: column;justify-content: space-around;height: 100%;">
    ...
    <div class="card-container" style="flex-direction: column;">
        <div class="icon">
            <a href="{{ url('/doctors.show_doctors/' . $clinic->id) }}">
                <i class="fa-solid fa-user-doctor" style="font-size: 400%;    color: #3061bc;cursor:pointer"></i>
            </a>
        </div>
        <div class="card-wrapper">
            @foreach ($clinic->doctors as $item2)
            <div class="card">
                <div class="card-header">
                    <div class="card-actions">
                        <a href="{{ url('doctors/' . $item2->id) }}" title="View Student"><i
                                class="fa-solid fa-circle-info"></i></a>
                        <a href="{{ url('/doctors/' . $item2->id . '/edit') }}" title="Edit Student"><i
                                class="fa-regular fa-pen-to-square"></i></a>
                        <form id="delete-form-{{ $item2->id }}" method="POST"
                            action="{{ route('doctors.destroy', $item2->id) }}" accept-charset="UTF-8"
                            style="display:inline">
                            @csrf
                            @method('DELETE')
                            <i class="fa-solid fa-trash-can trash_it" style="cursor:pointer"
                                data-form-id="delete-form-{{ $item2->id }}"></i>
                        </form>
                    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-actions">
                        <a href="{{ url('/doctors/' . $item2->id) }}" title="View Student"><i
                                class="fa-solid fa-circle-info"></i></a>
                        <a href="{{ url('/clinics/' . $item2->id . '/edit') }}" title="Edit Student"><i
                                class="fa-regular fa-pen-to-square"></i></a>
                        <form id="delete-form-{{ $item2->id }}" method="POST"
                            action="{{ route('clinics.destroy', $item2->id) }}" accept-charset="UTF-8"
                            style="display:inline">
                            @csrf
                            @method('DELETE')
                            <i class="fa-solid fa-trash-can trash_it" style="cursor:pointer"
