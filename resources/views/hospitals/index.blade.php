                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-actions">
                                ...
                                <a href="{{ url('/hospitals/' . $item->id) }}" title="View Student"><i class="fa-solid fa-circle-info"></i></a>

                                @if (session('user_type') == 2)
                                    
                                <a href="{{ url('/hospitals/' . $item->id . '/edit') }}" title="Edit Student"><i class="fa-regular fa-pen-to-square"></i></a>

                                <form id="delete-form-{{ $item->id }}" method="POST" action="{{ route('hospitals.destroy', $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="card-body">
                            <p class="card-text"><i class="fa-solid fa-circle-h"></i> {{ $item->name}}</p>
                            @if ($item->user)
                                
                            <p class="card-text"><i class="fa-regular fa-user"></i> {{ $item->user->name}}</p>
                            @endif
                            <p class="card-text"><i class="fa-solid fa-phone"></i> {{ $item->mobile }}</p>
                            <p class="card-title" style="overflow: auto"><i class="fa-solid fa-location-dot"></i>{{ $item->address }} </p>
                        </div>
                    </div>
                    @endforeach
                </div>
