...
@section('content')
    ...
    <div class="card" style="margin: 20px auto; width: 69%;">
        <div class="card-header">Create New operations</div>
        <div class="card-body">
            <form action="{{ url('operations') }}" method="post" dir="rtl" enctype="multipart/form-data">
                {!! csrf_field() !!}

                <!-- Hidden input to send the default image -->
                <input type="hidden" name="operations_pic" value="default_profile.png">

                <input type="file" name="operations_pic" id="operations_pic" style="display: none;">
                ...
                <label for="operation_name">operations</label></br>
                <input type="text" name="operation_name" id="operation_name" class="form-control"></br>
                
                <input type="hidden" name="facility_id" id="facility_id" value="{{ session('facility_id') }}" class="form-control">


                <div class="form-row" id="hospital_row" >
                    <label>تخصص</label>
                    ...
                    <select name="specialization_id" id="specialization_id" class="form-control">
                        ...
                        @foreach ($specialization as $item)
                            <option value="{{ $item->id }}">{{ $item->specialization_name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <script>
                $(document).ready(function() {
                    $('#clickableImage').on('click', function() {
                        $('#operations_pic').click();
                    });

                    $('form').on('submit', function(e) {
                        var fileInput = $('#operations_pic');
                        if (!fileInput.val()) {
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#operations_pic').on('change', function(event) {
