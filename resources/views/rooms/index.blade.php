                            ...
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <div class="card-actions">
                                        <a href="{{ url('/rooms/' . $item->id) }}" title="View Student"><i
                                                class="fa-solid fa-circle-info"></i></a>

                                        <a href="{{ url('/rooms/' . $item->id . '/edit') }}" title="Edit Student"><i
                                                class="fa-regular fa-pen-to-square"></i></a>

                                        <form id="delete-form-{{ $item->id }}" method="POST"
                                            action="{{ route('rooms.destroy', $item->id) }}" accept-charset="UTF-8"
                                ...
                                <div class="card-body">
                                    {{-- <h5 class="card-title"><i class="fa-solid fa-bed"></i></h5> --}}
                                    <p class="card-text"><i class="fa-solid fa-crown"></i>
                                        {{ $item->roomClass->roomsClass_name }}</p>
                                    <p class="card-text"><i class="fa-regular fa-building"></i> {{ $item->floor_num }}</p>
                                    <p class="card-text"><i class="fa-solid fa-list"></i> {{ $item->roomType->room_type_name }}</p>


                                    @if ($item->hospital && $item->facility_type === 1)
                                        <p class="card-text">
                                            <i class="fa-solid fa-circle-h"></i> {{ $item->hospital->hospital_name }}
                                        </p>
                                    @elseif ($item->clinic && $item->facility_type === 2)
                                        <p class="card-text">
                                            <i class="fa-solid fa-c"></i> {{ $item->clinic->clinic_name }}
                                        </p>
                                    @endif  
