                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="card-actions">
                                        <!-- Restore Appointment Form -->
                            

                                        <form id="restore-form-{{ $item->id }}" method="POST" action="{{ route('appointments.restore', $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            @csrf
                                            @method('POST')
                                        ...
                                        <form id="delete-form-{{ $item->id }}" method="POST" action="{{ route('appointments.destroy', $item->id) }}" accept-charset="UTF-8"
                                ...
                                <div class="card-body" style="overflow: auto;">
                                    ...
                                    <p class="card-text"><i class="fa-solid fa-calendar-days"></i>
                                        ...
                                        ->{{ \Carbon\Carbon::parse($item->end_date)->format('Y-m-d') }}</p>
                                    @php
                                        $bookingTypes = [
                                            1 => ['icon' => 'fa-bed', 'text' => 'إقامة'],
                                            2 => ['icon' => 'fa-scalpel', 'text' => 'عمليات'],
                                            3 => ['icon' => 'fa-baby', 'text' => 'حضانة'],
                                            4 => ['icon' => 'fa-heart-pulse', 'text' => 'عناية مشددة'],
                                        ];
                                    @endphp

                                    @if (isset($bookingTypes[$item->booking_type]))
                                        <p class="card-text">
                                            <i class="fa-solid {{ $bookingTypes[$item->booking_type]['icon'] }}"></i>
                                            {{ $bookingTypes[$item->booking_type]['text'] }}
                                        </p>
                                    @endif

                                    <p class="card-title"><i class="fa-solid fa-phone"></i>{{ $item->user->mobile }} </p>
                                    <p class="card-title"><i class="fa-solid fa-door-closed"></i>{{ $item->room_id }} </p>
                                </div>
                            </div>
                        @endforeach
                    </div>
